...
String query = "SELECT WEI.WIP_ENTITY_NAME,  MSI.segment1,  MSI.DESCRIPTION,  WOV.FIRST_UNIT_START_DATE,WDJ.START_QUANTITY,WDJ.STATUS_TYPE FROM WIP_ENTITIES WEI  INNER JOIN WIP_OPERATIONS_V WOV  ON WEI.WIP_ENTITY_ID = WOV.WIP_ENTITY_ID INNER JOIN WIP_DISCRETE_JOBS WDJ  ON WEI.WIP_ENTITY_ID = WDJ.WIP_ENTITY_ID  INNER JOIN MTL_SYSTEM_ITEMS MSI ON WEI.PRIMARY_ITEM_ID = MSI.INVENTORY_ITEM_ID  WHERE     WOV.OPERATION_SEQ_NUM = 10   AND WOV.OPERATION_CODE = 'PRMX'     AND WDJ.PROGRAM_ID IS NULL    AND WDJ.REQUEST_ID IS NULL AND (WDJ.STATUS_TYPE = 4 OR WDJ.STATUS_TYPE =3) AND FIRST_UNIT_START_DATE between sysdate-15 AND sysdate AND WOV.OPERATION_CODE  like '%"+paramDepartment+"%' ORDER BY FIRST_UNIT_START_DATE ASC"; 
